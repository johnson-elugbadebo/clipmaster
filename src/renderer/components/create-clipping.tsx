...
const CreateClipping = ({ onSubmit }: CreateClippingProps) => {
  ...
  return (
    <form
      ...
      <input value={value} className='w-full border-none' placeholder='Type a new clipping and press "Enter" to save' id='new-clipping' ref={ref} onChange={(e) => setValue(e.target.value)} />
      <button type='submit' disabled={!value} className='flex-1 w-full rounded-none'>
        Save to Clipboard
      </button>
    </form>
  );
