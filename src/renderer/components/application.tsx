...
const Application = () => {
  ...
  return (
    <main className='flex flex-col w-screen h-screen'>
      <header className='flex items-center h-10 font-semibold text-white bg-primary-400' id='title-bar'>
      ...
      <section className='flex flex-col h-full gap-2 p-4 overflow-y-scroll'>
        {clippings.map((clipping) => (
          <Clipping key={clipping.id} id={clipping.id} value={clipping.value} onRemove={removeClipping} onCopy={window.api.writeToClipboard} />
        ))}
      </section>
      <CopyFromClipboard
        onClick={async () => {
